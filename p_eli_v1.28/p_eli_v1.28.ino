...
const int FRONT_WARN = 9;      // cm — early warning: stop self-stabilize and check for turn(6 works but minor problems)
...
// ---------------------------------------------------------------
// FIX: Self-stabilize now exits early if front wall is approaching
// Returns false if it bailed out due to front obstacle, true otherwise
// ---------------------------------------------------------------
bool moveForwardSelfStabilize(long distLeft, long distRight) {
  ...
  if (distLeft < WALL_THRESHOLD && distRight < WALL_THRESHOLD) {
    ...
    // KEY FIX: Check front sensor before committing to stabilization move
    long distFront = getDistance(trigFront, echoFront);
    if (distFront <= FRONT_WARN) {
      // Front wall (or turn) is close — stop stabilizing, hand back to loop()
      stopMotors();
      return false;
    }
