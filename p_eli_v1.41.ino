...
const int PRE_TURN_NUDGE_MOVING = 400; // FIX: shorter nudge when left is detected while moving
...
void loop() {
  ...
  if (!leftWall && leftLockout == 0) {
    if (isOpenMajority(trigLeft, echoLeft)) {
      stopMotors();                        // FIX: stop before nudging so we
      delay(80);                           //      don't carry extra momentum
      nudge(PRE_TURN_NUDGE_MOVING);        // FIX: short 400ms nudge, not 1050ms
      turnLeft();
    } else {
