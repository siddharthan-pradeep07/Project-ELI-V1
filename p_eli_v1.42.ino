...
const int PRE_TURN_NUDGE    = 1050;   // used only when already STOPPED at a wall
const int PRE_TURN_NUDGE_MOVING = 550; // nudge to centre axle at junction — tune in ~50ms steps
...
// ---------------------------------------------------------------
// Drive dead-straight for ms milliseconds — NO wall correction.
// Use this when approaching a turn so the sensor asymmetry at
// a junction opening cannot steer the robot off-angle.
// ---------------------------------------------------------------
void blindStraight(int ms) {
  motorA(BASE_SPEED);
  motorB(BASE_SPEED);
  delay(ms);
  stopMotors();
...
//
// FIX: When moving forward and a left opening is detected,
// use PRE_TURN_NUDGE_MOVING (shorter) instead of PRE_TURN_NUDGE.
// The old code used a 1050ms nudge even while already moving,
// which caused the robot to overshoot past the junction and
// miss or clip the left turn.
// ---------------------------------------------------------------
void loop() {
  ...
  if (!leftWall && leftLockout == 0) {
    if (isOpenMajority(trigLeft, echoLeft)) {
      stopMotors();                        // stop before nudging
      delay(80);
      blindStraight(PRE_TURN_NUDGE_MOVING); // drive straight, no stabilization
      turnLeft();
    } else {
  ...
  // -------------------------------------------------------
    ...
    stopMotors();
    delay(100);
    nudge(PRE_TURN_NUDGE);   // full nudge OK here — robot was already stopped
    stopMotors();
