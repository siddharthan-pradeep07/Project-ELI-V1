...
// ---------------------------------------------------------------
// CREEP FORWARD — moves toward front wall one small step at a time,
// checking LEFT and RIGHT at every step.
// Returns: 'L' if left opened, 'R' if right opened, 'F' if hit front.
// This prevents the robot from overshooting a junction while creeping.
// ---------------------------------------------------------------
char creepToFrontCheckingSides() {
  while (true) {
    long dFront = getDistance(trigFront, echoFront);
    long dLeft  = getDistance(trigLeft,  echoLeft);
    long dRight = getDistance(trigRight, echoRight);

    Serial.printf("  [creep] L:%ld F:%ld R:%ld\n", dLeft, dFront, dRight);

    // Front reached
    if (isFrontBlocked(dFront)) {
      stopMotors();
      delay(80);
      return 'F';
    }

    // Left opened while creeping — bail immediately
    if (dLeft >= WALL_THRESHOLD && confirmLeftOpen()) {
      stopMotors();
      delay(80);
      return 'L';
    }

    // Right opened while creeping — bail immediately
    if (dRight >= WALL_THRESHOLD && confirmRightOpen()) {
      stopMotors();
      delay(80);
      return 'R';
    }
...
//  3. Front blocked               → creep forward checking all sides
//                                   L opened → turn left
//                                   R opened → turn right
//                                   Truly blocked → turn around
// ---------------------------------------------------------------
void loop() {
  ...
  // --- FRONT BLOCKED: creep forward and watch all sides ---
  else {
    nudge(PRE_TURN_NUDGE);  // nudge into the junction centre
    char result = creepToFrontCheckingSides();

    if (result == 'L') {
      // Left opened during creep — take it (left-hand rule)
      turnLeft();
    } else if (result == 'R') {
      // Right opened during creep — take it
      turnRight();
    } else {
      // Front confirmed, sides confirmed closed — true dead end
      turnAround();
    }
  }
