...
void loop() {
  ...
  // ---------------------------------------------------------------
  // STRICT LEFT-HAND WALL FOLLOWING PRIORITY:
  //
  // Scenario                   | L | F | R | Action
  // ---------------------------|---|---|---|-------------------
  // Left open                  | 0 | x | x | Turn Left
  // Straight (left wall)       | 1 | 0 | x | Go Straight
  // Right only (left+front)    | 1 | 1 | 0 | Turn Right
  // Dead end (all walls)       | 1 | 1 | 1 | Turn Around
  // ---------------------------------------------------------------

  if (!leftWall) {
    // Left is open — confirm it's not sensor noise, then turn left
    if (confirmNoWall(trigLeft, echoLeft)) {
      turnLeft();
    } else {
      moveForward(); // noise, keep going
    }
  }
  else if (!frontWall) {
    // Left wall present, front clear — go straight regardless of right wall
    ...
    // All walls — dead end, turn around
    turnAround();
  }
