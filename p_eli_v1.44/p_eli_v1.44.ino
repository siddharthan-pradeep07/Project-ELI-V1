...
// ---------------------------------------------------------------
// PD Wall-Following Controller Constants
// ---------------------------------------------------------------
const int KP             = 5;    // Proportional gain (reduced to prevent oscillation)
const int KD             = 8;    // Derivative gain (dampens overshoot)
const int MAX_CORRECTION = 70;
const int LEFT_TARGET    = 8;    // Ideal distance from left wall (cm)
const int RIGHT_TARGET   = 8;    // Ideal distance from right wall (cm)

const int LEFT_LOCKOUT_CYCLES = 6;
int leftLockout = 0;
int lastError   = 0;  // PD derivative state — reset on every turn
...
// ---------------------------------------------------------------
// PD Wall-Following: unified signed error convention
//   Positive error → robot drifted right → correct left
//   Negative error → robot drifted left  → correct right
// ---------------------------------------------------------------
void moveForwardSelfStabilize(long distLeft, long distRight) {
  ...
  if (hasLeft && hasRight) {
    // Corridor: balance between both walls symmetrically
    int leftError  = (int)distLeft  - LEFT_TARGET;
    int rightError = RIGHT_TARGET   - (int)distRight;
    error = leftError - rightError;
  } else if (hasLeft) {
    // Only left wall visible: maintain target distance from it
    error = (int)distLeft - LEFT_TARGET;
  } else if (hasRight) {
    // Only right wall visible: maintain target distance from it
    error = RIGHT_TARGET - (int)distRight;
  }
  ...
  lastError = error;
  int correction = constrain(error * KP + derivative * KD, -MAX_CORRECTION, MAX_CORRECTION);

  // Positive correction → slow right motor (B), speed up left (A)
...
// ---------------------------------------------------------------
// Turn routines — reset PD state on every turn to prevent
// derivative spike from stale pre-turn error value
// ---------------------------------------------------------------
void turnLeft() {
  lastError = 0;
  spinLeft(SPIN_90_MS);
