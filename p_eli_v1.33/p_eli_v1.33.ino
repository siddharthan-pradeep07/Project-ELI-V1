...
const int WALL_FOLLOW_DIST =  7; // cm — desired gap to left wall (single-wall mode)
...
// ---------------------------------------------------------------
// SELF STABILIZE — both walls present, center between them
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
// SINGLE WALL FOLLOW — only left wall visible
// Hug left wall at WALL_FOLLOW_DIST
// ---------------------------------------------------------------
void moveForwardHugLeft(long distLeft) {
  int error      = distLeft - WALL_FOLLOW_DIST; // +ve = too far, steer left
  int correction = constrain(error * 5, -40, 40);
  motorA(BASE_SPEED - correction);
  motorB(BASE_SPEED + correction);
}
...
void loop() {
  ...
  if (!leftWall && leftLockout == 0) {
    if (confirmLeftOpen()) {
      ...
      // Noise — keep hugging left wall or going straight
      if (leftWall && rightWall) moveForwardSelfStabilize(distLeft, distRight);
      else driveForward();
    }
  }
  ...
  // --- FRONT CLEAR: go straight ---
    ...
    if (leftWall && rightWall) {
      // Both walls — self-stabilize (keeps robot centered precisely)
      moveForwardSelfStabilize(distLeft, distRight);
    } else if (leftWall && !rightWall) {
      // Only left wall — hug it, ignore missing right
      moveForwardHugLeft(distLeft);
    } else {
      // No useful wall reference — drive straight
      driveForward();
    }
  }
